...
namespace SLS {
void ImageFileProcessor::loadImages(const std::string &folder,
                                    std::string prefix, size_t numDigits,
                                    size_t startIdx, std::string suffix) {
...
void ImageFileProcessor::loadConfig(const std::string &distMat,
...
Ray ImageFileProcessor::getRay(const size_t &x, const size_t &y) {
  ...
  ray.dir.x = (undistorted.x - (float)params_[CAMERA_MAT].at<double>(0, 2)) /
              (float)params_[CAMERA_MAT].at<double>(0, 0);
  ray.dir.y = (undistorted.y - (float)params_[CAMERA_MAT].at<double>(1, 2)) /
              (float)params_[CAMERA_MAT].at<double>(1, 1);
  ray.dir.z = 1.0;
  ray.dir.w = 0.0;
...
Ray ImageFileProcessor::getRay(const size_t &pixelIdx) {
  ...
  ray.origin = camTransMat_ * glm::vec4(0.0, 0.0, 0.0, 1.0);
  ray.dir.x = (undistorted.x - params_[CAMERA_MAT].at<double>(0, 2)) /
              params_[CAMERA_MAT].at<double>(0, 0);
  ray.dir.y = (undistorted.y - params_[CAMERA_MAT].at<double>(1, 2)) /
              params_[CAMERA_MAT].at<double>(1, 1);
  ray.dir.z = 1.0;
  ray.dir.w = 0.0;
  ray.dir = camTransMat_ * ray.dir;
  ray.dir = glm::normalize(ray.dir);
...
glm::vec2
ImageFileProcessor::undistortPixel(const glm::vec2 &distortedPixel) const {
  double k[5] = {0.0};
...
Buckets ImageFileProcessor::generateBuckets(size_t projWidth, size_t projHeight,
                                            size_t requiredNumFrames) {
  Buckets bkts;
  ...
  for (size_t i = 0; i < xTimesY; i++) {
    if (!queryMask(i))
      continue;
    getNextFrame();
    getNextFrame(); // Skip first two frames
